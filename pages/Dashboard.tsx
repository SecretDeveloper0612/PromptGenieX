    ...
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          className={`transition-all duration-200 ${interactive ? 'hover:scale-125' : ''} ${(hovered !== null ? star <= hovered : star <= Math.round(rating))
...
export const Dashboard: React.FC = () => {
  ...
  const scrollTools = (direction: 'left' | 'right') => {
    if (toolScrollRef.current) {
      ...
      toolScrollRef.current.scrollBy({
  ...
  const toggleRecording = async () => {
    ...
    if (!SpeechRecognition) {
      addLog("Voice engine not supported in this browser.", "warn");
      return;
    }
    ...
    // --- NEW: FORCE PERMISSION PROMPT ---
    addLog("Initializing mic hardware...", "info");
    try {
      // This forces the 'Allow' popup even if the browser is being stubborn
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      // Stop the stream immediately, it was just for the permission prompt
      stream.getTracks().forEach(track => track.stop());
    } catch (err: any) {
      console.error('Mic Access Error:', err);
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        addLog("Access Denied. Please tap the lock icon in URL bar and enable mic.", "warn");
      } else {
        addLog("Hardware error or busy. Try refreshing.", "warn");
      }
      return;
    }
    // ------------------------------------
    ...
    recognition.onstart = () => {
      setIsRecording(true);
      addLog("Voice engine active. Speak now...", "info");
      playSound('click');
    };
    ...
    recognition.onerror = (event: any) => {
      if (event.error === 'not-allowed') {
        addLog("Permission revoked. Check URL settings.", "warn");
      } else if (event.error === 'no-speech') {
        // Handle silently
      } else {
        addLog(`Engine error: ${event.error}`, "warn");
      }
      setIsRecording(false);
    };
    ...
    recognition.onend = () => {
      setIsRecording(false);
      addLog("Voice engine standby.", "success");
    };
    ...
    <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-8 md:py-12 min-h-screen relative">
      ...
      {activeTab === 'workspace' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-9 space-y-12">
            ...
            <div className="space-y-6" data-tour="tools">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
                ...
                <div className="flex items-center gap-2 p-1 bg-white/5 border border-white/10 rounded-xl overflow-hidden self-start md:self-auto">
                  {(['All', 'Logic', 'Visual', 'Audio', 'Video'] as ToolCategory[]).map((cat) => (
                    <button
                      key={cat}
                      onClick={() => { setToolCategoryFilter(cat); setToolSearchQuery(''); }}
                      className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${toolCategoryFilter === cat ? 'bg-purple-500 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                    >
                      {cat}
                    </button>
                  ))}
              ...
              <div className="relative group/nav">
                <div className="flex overflow-x-auto gap-4 pb-4 px-2 no-scrollbar" ref={toolScrollRef}>
                  {filteredTools.map((tool) => (
                    <button
                      key={tool.id}
                      onClick={() => setSelectedTool(tool.id)}
                      className={`min-w-[160px] p-6 rounded-3xl border transition-all flex flex-col items-center gap-4 ${selectedTool === tool.id ? 'bg-purple-500/10 border-purple-500 shadow-2xl scale-[1.02]' : 'bg-white/5 border-white/5 hover:bg-white/10'
                        }`}
                    >
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${selectedTool === tool.id ? 'bg-purple-500 text-white shadow-lg shadow-purple-500/30' : 'bg-black/40 text-gray-600 group-hover:text-gray-400'}`}>
                        {tool.icon}
                      </div>
                ...
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/60 border border-white/10 text-white flex items-center justify-center backdrop-blur-md opacity-0 group-hover/nav:opacity-100 transition-all hover:bg-purple-500 hover:border-purple-500 z-10"
                >
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button
                  onClick={() => scrollTools('right')}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/60 border border-white/10 text-white flex items-center justify-center backdrop-blur-md opacity-0 group-hover/nav:opacity-100 transition-all hover:bg-purple-500 hover:border-purple-500 z-10"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-12">
                  <div className="md:col-span-8 p-10 border-r border-white/5" data-tour="input">
                    ...
                    <div className="relative">
                      ...
                      />
                      <div className="absolute bottom-4 right-2 flex items-center gap-3">
                        <button
                          onClick={toggleRecording}
                          className={`p-3 rounded-2xl transition-all shadow-lg flex items-center gap-2 border ${isRecording
                        ...
                        {input.trim() && (
                          <button
                            ...
                            disabled={isRefining}
                            className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-2xl text-purple-400 hover:bg-purple-500 hover:text-white transition-all shadow-lg flex items-center gap-2"
                          >
          ...
          <aside className="lg:col-span-3 space-y-8 sticky top-24 self-start" data-tour="sidebar">
...
const VaultCard: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`w-full p-6 rounded-[2rem] border text-left transition-all relative overflow-hidden group/card ${isActive
  ...
  >
    ...
    <div className="mt-4 flex justify-end items-center gap-2 opacity-0 group-hover/card:opacity-100 transition-opacity relative z-10">
      <button
        ...
        className={`p-2 rounded-lg border transition-all ${item.isFrozen
      ...
      <div className="flex items-center gap-2">
        {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
          <button
            ...
            className={`w-12 h-12 rounded-2xl border transition-all text-sm font-black ${currentPage === page
...
const TemplateCard: React.FC<{
  ...
  >
    <div className="absolute top-0 right-0 p-10 flex flex-col items-end gap-2">
      <div className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${tmpl.complexity === 'Elite' ? 'bg-orange-500/10 border-orange-500/20 text-orange-400' :
